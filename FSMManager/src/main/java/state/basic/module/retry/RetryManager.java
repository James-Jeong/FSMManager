...
public class RetryManager {
     ...
     * 1) 현재 재시도 횟수가 재시도 제한 횟수 미만이면, RetryStatus.ONGOING 상태를 반환
     * 2) 현재 재시도 횟수가 재시도 제한 횟수 이상이면, RetryStatus.COMPLETED 상태를 반환
     * 3) RetryUnit 이 삭제되었다면, RetryStatus.REMOVED 상태를 반환
     * 4) 재시도 제한 횟수가 0 이면 재시도 하지 않으므로, null 을 반환
    ...
    public RetryStatus checkRetry(String key) {
        ...
        if (retryUnit == null) {
            logger.warn("[{}] Not found the RetryUnit. (key={})",
                    ResultCode.FAIL_GET_RETRY_UNIT, key
            );
            return RetryStatus.IDLE;
        } else if (retryUnit.getRetryCountLimit() <= 0) {
            logger.warn("[{}] Retry limit count is not positive. (key={})",
    ...
    public RetryUnit addRetryUnit(String key, int retryCountLimit, int initialRetryCount) {
        try {
            ...
            if (retryUnitMap.putIfAbsent(key, new RetryUnit(key, retryCountLimit, initialRetryCount)) == null) {
                logger.info("[{}] Success to add a RetryUnit. ({})",
    ...
    public void removeRetryUnit(String key) {
        try {
            ...
            if (retryUnit != null) {
                logger.info("[{}] Success to remove the RetryUnit. ({})",
